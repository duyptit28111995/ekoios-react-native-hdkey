...
HDKey.prototype.deriveChild = function (index) {
  ...
  var I = Buffer.from(crypto.HmacSHA512(data.toString(), this.chainCode.toString()).toString(), 'hex')
  var IL = I.slice(0, 32)
  var IR = I.slice(32)
...
HDKey.fromMasterSeed = function (seedBuffer, versions) {
  var I = Buffer.from(crypto.HmacSHA512(seedBuffer.toString(), 'Bitcoin seed').toString(), 'hex')
  var IL = I.slice(0, 32)
  var IR = I.slice(32)
...
function hash160 (buf) {
  var sha = Buffer.from(crypto.SHA256(buf).toString(), 'hex')
  return Buffer.from(crypto.RIPEMD160(sha).toString(), 'hex')
}
